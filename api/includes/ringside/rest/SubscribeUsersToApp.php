 ...
 */
class SubscribeUsersToApp extends Api_DefaultRest
{
	private $uid;
	private $nid;
	private $aid;
	...
	public function validateRequest( ) {
		...
		$this->friends = $this->getRequiredApiParam( 'friends' );
	...
	public function execute() {
		...
		foreach( $friendUids as $friendUid ) {
			$friends[] = array( 'uid'=>$friendUid, 'nid'=>$this->nid ); //TODO: Support case where friend is not a user in the deployed network (i.e. in the users table)
		}

		$result = Api_Bo_Subscriptions::createFriendsAppSubscription( $this->uid, $this->nid, $this->aid, $this->planid, $this->ccn,
    		$this->cctype, $this->expdate, $friends, $this->firstname, $this->lastname, $this->email, $this->phone );
