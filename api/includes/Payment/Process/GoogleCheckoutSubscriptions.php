...
class Payment_Process_GoogleCheckoutSubscriptions extends Payment_Process_Common
{
    /**
    ...
    function __construct($options = false)
    {
        ...
        $this->_driver = 'GoogleCheckoutSubscriptions';
        $this->_makeRequired( 'login', 'password', 'refId', 'intervalLength', 
        	'intervalUnit', 'startDate', 'totalOccurrences', 'amount' );
    }

    function Payment_Process_GoogleCheckoutSubscriptions( $options = false )
    {
...
//        error_log( 'GoogleCheckoutSubscriptions->process(),Ê_data: ' . print_r( $this->_data, true ) );
		...
		if ( function_exists( 'curl_init' ) ) {
			...
			curl_setopt($ch, CURLOPT_URL, $this->_options[ 'authorizeUri' ] );
	      	...
	      	curl_setopt($ch, CURLOPT_USERAGENT, 'PEAR Payment_Process_GoogleCheckoutSubscriptions 0.1' );
...
//                    'User-Agent: PEAR Payment_Process_GoogleCheckoutSubscriptions 0.1 (non-curl) '.phpversion()."\r\n".
...
//
        ...
        error_log( 'GoogleCheckoutSubscriptions->process(),Ê_responseBody: ' . "\n" . print_r( $this->_responseBody, true ) );
    ...
    function _prepareXml()
    {
        ...
        $save_error_level = error_reporting(0);
...
class Payment_Process_Result_GoogleCheckoutSubscriptions extends Payment_Process_Result {
    ...
    function Payment_Process_Response_GoogleCheckoutSubscriptions( $rawResponse )
    {
    ...
    function parse()
    {
//    	error_log( 'GoogleCheckoutSubscriptionsResponse->parse(), response: ' . $this->_rawResponse );
        $save_error_level = error_reporting(0);
