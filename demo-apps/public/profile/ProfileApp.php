...
//print get_include_path();
//  print dirname(dirname(dirname(dirname(__FILE__)))).'/LocalSettings.php';
// 
// $lsError = error_reporting( E_ERROR );
// $ls1 = require_once(dirname(dirname(dirname(dirname(__FILE__)))).'/LocalSettings.php');
...
class ProfileApp 
{
	...
	public function getEditFormData(){
		...
		$dateParts=date_parse($user->userbasicprofile->dob );
		$birthDayOfMonth=0;
		$birthMonth=0;
		$birthYear=1909;
		if(array_key_exists('day',$dateParts))  {$birthDayOfMonth=$dateParts['day'];}
		if(array_key_exists('month',$dateParts))  {$birthMonth=$dateParts['month'];}
...
//		var_dump($req);
	...
	public function getStatusBlock($flavor='wide'){
	    global $user;
	    $userAppList = $this->restClient->users_getInfo($user, array('status'));
	    
        $statusRow=$userAppList[0];
	...
	public function getProfileInfoWideBlock(){
        ...
        // $user=$user->find($this->uid);
        $userInfo = $this->restClient->users_getInfo($user, array('political','relationship_status','religion','birthday'));
        //error_log(var_export($userInfo,1));
        $userInfo = $userInfo[0];
        $relationshipStatus = $userInfo["relationship_status"];
		...
		$status_fbml[]="<b>Political Views: </b>$political<br>\n";
		$status_fbml[]="<b>Religious Views: </b>$religion<br>\n";
		$status_fbml[]="<b>Birthday:</b> $dob<br>\n";
		$status_fbml[]="<br>\n";
		$status_fbml[]="</div><br>";
		return $status_fbml;
	}
