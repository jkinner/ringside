...
class fbTabItemHandler {
   ...
   function doBody($application, &$parentHandler, $args, $body ) {
      ...
      if ( strcasecmp( $align, "right") != 0 && strcasecmp( $align, "left") != 0 ) {
         $align = 'left';
      } 
      $selected = isset( $args['selected'] ) ? $args['selected'] : 'false';
      if ( strcasecmp( $selected, "true") != 0 && strcasecmp( $selected, "false") != 0 ) {
         $selected = 'false';
      } 
      
      if ( $parentHandler != null && method_exists($parentHandler, 'addItem') ) {
         return $parentHandler->addItem( $href, $title, $align, $selected );
      } 
   }
