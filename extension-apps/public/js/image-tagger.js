...
//              inputElem.parentNode.parentNode.style.top = (config.y + config.h) + 'px';
				var i = 0;
				if(config.y != null && !isNaN(config.y))
				{
					//alert('setting i to ' + config.y)
					i += config.y;
				}
				if(config.h != null && !isNaN(config.h))
				{
					//alert('setting i to ' + config.h)
					i += config.h;
				}	
				//alert(i);
				inputElem.parentNode.style.top = i + 'px';
				//inputElem.parentNode.parentNode.style.top = i + 'px';
      }
...
//    _tagPlane.style.border="solid";
    _imgScopeNode.parentNode.parentNode.replaceChild(_tagPlane, tagPlane);
    _tagPlane.appendChild(_imgScopeNode);
  ...
  function handleGetTagsResponse() {
    if ( req.readyState == 4 ) {
      if ( req.responseXML != null ) {
        if ( req.responseXML.documentElement == null || req.responseXML.documentElement.nodeName != 'tags' ) {
...
//    addTagElem.style.zIndex = 75;
    addEventHandler(addTagElem,  'click', function (event) {if ( ! event ) { event = window.event; } showCropper(_img.id, defineConfig(this, event), this);});
    _tagList.appendChild( addTagElem );
    if ( _img.parentNode.nodeName == 'A' || _img.parentNode.nodeName == 'a' ) {
      _imgScopeNode = _img.parentNode;
      // TODO: Once we work out how to enable drag/drop while preventing the anchor click from activating, remove this code
      _imgScopeNode.parentNode.replaceChild(_img, _imgScopeNode);
      _imgScopeNode = _img;
    } else {
      _imgScopeNode = _img;
	  }
    _imgScopeNode.parentNode.replaceChild(_tagPlane, _imgScopeNode);
    _tagPlane.appendChild(_imgScopeNode);
    _tagPlane.appendChild(_tagList);
    getTags();
  }
