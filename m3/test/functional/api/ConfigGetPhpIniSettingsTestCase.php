...
require_once('m3/rest/ConfigGetPhpIniSettings.php');
require_once('ringside/m3/util/PhpSettings.php');

class ConfigGetPhpIniSettingsTestCase extends BaseM3ApiTestCase
{
    public function testGetPhpIniSettings()
    {
        $_apiCall = $this->initRest( new ConfigGetPhpIniSettings(), null);
        $_results = $_apiCall->execute();
        $this->assertTrue(is_array($_results));
        $this->assertTrue(is_array($_results['php_ini_setting']));
        $_results = $_results['php_ini_setting'];
